...
import { ENEMY_BASE_TEMPLATES, SPECIAL_ABILITY_CHANCES, ATTACK_PATTERNS } from "../../../data/EnemyTemplates";
...
export class AttackAbilityComponent {
     ...
     * ランダムな攻撃能力を生成
     */
    public generateRandomAttackAbility(enemyType: EnemyType, playerLevel: number = 1): AttackAbility {
    ...
    public generateEliteAttackAbility(baseAttack: AttackAbility): AttackAbility {
        return {
            bulletType: baseAttack.bulletType === 'single' ? 'spread' : baseAttack.bulletType,
            ...
            specialEffects: [
                ...
                this.randomProvider.randomChoice(['piercing', 'explosive'] as SpecialEffect[])
            ]
        };
    }
    ...
    public calculateBulletDirections(
        ...
        switch (attackAbility.bulletType) {
            ...
            case 'spread':
                ...
                const angleStep = spreadConfig.angleSpread / (attackAbility.bulletCount - 1);
            ...
            case 'burst':
                ...
                const burstAngleStep = burstConfig.burstSpread / (attackAbility.bulletCount - 1);
                const burstStartAngle = baseAngle - burstConfig.burstSpread / 2;

                for (let i = 0; i < attackAbility.bulletCount; i++) {
                    directions.push({
                        angle: burstStartAngle + (burstAngleStep * i),
                        speed: attackAbility.bulletSpeed * this.randomProvider.randomRange(0.9, 1.1)
                    });
                }
                break;
    ...
    public getAttackDescription(ability: AttackAbility): string {
        ...
        if (ability.specialEffects.length > 0) {
            const effects = ability.specialEffects.map(effect => effectDescriptions[effect]).join(', ');
            description += ` [${effects}]`;
        }
     ...
     * 攻撃の脅威レベルを計算
     */
    public calculateThreatLevel(ability: AttackAbility, attackPower: number): number {
     ...
     * 攻撃パターンの詳細情報を取得
     */
    public getAttackInfo(ability: AttackAbility, baseAttackPower: number): {
        ...
        const estimatedDPS = (actualPower * ability.bulletCount) / (actualFireRate / 1000);
