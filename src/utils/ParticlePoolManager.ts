...
export class ParticlePoolManager {
  ...
  public adjustPoolSize(poolName: string, newSize: number): void {
    ...
    if (newSize > config.maxSize) {
      console.warn(
        `Requested size ${newSize} exceeds max size ${config.maxSize} for pool '${poolName}'`
      );
      return;
    }
  ...
  public getStats(): ParticlePoolStats {
    ...
    const poolUtilization =
      totalParticles > 0 ? activeParticles / totalParticles : 0;
    const memoryEfficiency =
      this.totalRequests > 0 ? this.totalHits / this.totalRequests : 0;
  ...
  public optimizePools(): void {
    this.poolStats.forEach((stats, poolName) => {
      ...
      // 使用率が30%を下回る場合、プールサイズを減少
        ...
        stats.utilizationRate < 0.3 &&
        stats.poolSize > config.initialSize
      ) {
  ...
  public logStats(): void {
    ...
    console.log(
      `📈 Pool Utilization: ${(stats.poolUtilization * 100).toFixed(1)}%`
    );
    console.log(
