...
export class GameModeManager {
  ...
  applyModifiers<T extends Record<string, number>>(baseValues: T): T {
    ...
    if ('enemySpeed' in result && typeof result['enemySpeed'] === 'number') {
      this.applyModifierSafely(
        result,
        'enemySpeed',
        modifiers.enemySpeedMultiplier
      );
    }
    ...
    if ('spawnRate' in result && typeof result['spawnRate'] === 'number') {
      this.applyModifierSafely(
        result,
        'spawnRate',
        modifiers.enemySpawnRateMultiplier
      );
    }
  ...
  calculateReward(baseReward: {
    ...
    return {
      score: baseReward.score
        ? Math.floor(baseReward.score * modifiers.scoreMultiplier)
        : baseReward.score,
      coins: baseReward.coins
        ? Math.floor(baseReward.coins * modifiers.coinMultiplier)
        : baseReward.coins,
      experience: baseReward.experience
        ? Math.floor(baseReward.experience * modifiers.experienceMultiplier)
