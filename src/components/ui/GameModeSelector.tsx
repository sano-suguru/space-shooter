...
export const GameModeSelector: React.FC<GameModeSelectorProps> = ({
  ...
  // 個別モードの統計を取得
  const getIndividualModeStats = (modeId: string): { gamesPlayed: number; highScore: number } => {
    const gameModeStats = playerProfile.gameModeStats || { gamesPlayedByMode: {}, highScoresByMode: {} };
  ...
  // 通知表示
  const showNotification = (type: 'success' | 'error' | 'unlock', message: string) => {
  ...
  // 通知アイコンを取得
  const getNotificationIcon = (type: 'success' | 'error' | 'unlock'): string => {
    switch (type) {
      case 'success': return '🎮';
      case 'unlock': return '🔓';
      case 'error': return '❌';
  ...
  return (
    <>
      ...
      {/* 通知表示 */}
      {notification && (
        <div className={`mode-notification ${notification.type} ${notification.isVisible ? '' : 'fade-out'}`}>
