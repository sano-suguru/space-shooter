...
export class Boss extends GameObject {
  ...
  private shieldLayers: Array<{
    radius: number;
    rotation: number;
    opacity: number;
    speed: number;
  }> = [];
  private geometricPanels: Array<{
    x: number;
    y: number;
    size: number;
    rotation: number;
  ...
  private thrusterNodes: Array<{
    x: number;
    y: number;
    size: number;
    pulse: number;
  }> = [];
  private energyBeams: Array<{
    angle: number;
    length: number;
    intensity: number;
    rotation: number;
  ...
  public update(deltaTime: number): void {
    if (this.y < 50) {
      ...
      const nextX =
        this.x +
        this.moveDirection * this.config.boss.movementSpeed * deltaTime;
  ...
  private drawMainBody(ctx: CanvasRenderingContext2D): void {
    ...
    for (let i = 0; i < vertices; i++) {
      ...
      const wave1 =
        Math.sin(this.animationPhase * 0.7 + angle * 3) * (baseRadius * 0.15);
      const wave2 =
        Math.cos(this.animationPhase * 1.1 + angle * 5) * (baseRadius * 0.08);
  ...
  private drawGeometricPanels(ctx: CanvasRenderingContext2D): void {
    this.geometricPanels.forEach(panel => {
      ...
      const alpha = Math.floor(glowIntensity * 255)
        .toString(16)
        .padStart(2, '0');
  ...
  private drawEnergyBeams(ctx: CanvasRenderingContext2D): void {
    this.energyBeams.forEach(beam => {
      ...
      ctx.beginPath();
      ctx.moveTo(
        Math.cos(beam.angle) * startRadius,
        Math.sin(beam.angle) * startRadius
      );
      ctx.lineTo(
        Math.cos(beam.angle) * endRadius,
        Math.sin(beam.angle) * endRadius
      );
      ctx.stroke();
