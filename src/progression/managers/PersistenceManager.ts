...
export class PersistenceManager {
    ...
    private static validateProfile(profile: PlayerProfile): PlayerProfile {
        ...
        const validatedProfile: PlayerProfile = {
            totalGamesPlayed: profile.totalGamesPlayed ?? defaultProfile.totalGamesPlayed,
            ...
            equippedUpgrades: profile.equippedUpgrades && typeof profile.equippedUpgrades === 'object' 
                ? profile.equippedUpgrades : {},
            completedAchievements: Array.isArray(profile.completedAchievements) ? profile.completedAchievements : [],
    ...
    private static validateStats(stats: PlayerStats | undefined): PlayerStats {
        ...
        return {
            enemiesDestroyed: Math.max(0, stats.enemiesDestroyed ?? defaultStats.enemiesDestroyed),
            bossesDefeated: Math.max(0, stats.bossesDefeated ?? defaultStats.bossesDefeated),
            maxWaveReached: Math.max(0, stats.maxWaveReached ?? defaultStats.maxWaveReached),
            powerupsCollected: Math.max(0, stats.powerupsCollected ?? defaultStats.powerupsCollected),
            ...
            playStreakDays: Math.max(1, stats.playStreakDays ?? defaultStats.playStreakDays)
        };
    }
     ...
     */
    private static migrateProfile(profile: PlayerProfile, oldVersion: string | null): PlayerProfile {
        console.log(`プロファイルを${oldVersion}から${this.CURRENT_VERSION}に移行中...`);
