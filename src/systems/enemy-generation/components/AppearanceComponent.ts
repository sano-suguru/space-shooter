...
export class AppearanceComponent {
   ...
   * エリート敵用の強化された外見を生成
   */
  public generateEliteAppearance(
  ...
  public drawEnemy(
    ...
    const glowPulse =
      Math.sin(animationPhase * config.animationSpeed * 2) * 0.3 + 0.7;
    ...
    this.drawGlowEffect(
      ...
      config,
      actualSize,
      config.glowIntensity * glowPulse
    );
  ...
  private drawGlowEffect(
    ...
    glowGradient.addColorStop(
      0,
      this.addAlpha(config.primaryColor, intensity * 0.3)
    );
    glowGradient.addColorStop(
      0.5,
      this.addAlpha(config.primaryColor, intensity * 0.1)
    );
