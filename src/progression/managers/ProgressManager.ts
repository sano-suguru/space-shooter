...
export class ProgressManager {
  ...
  public getPlayerStatsForEnemyGeneration(): {
    ...
    const averageWave =
      recentGames > 0 ? this.currentSession.waveReached / recentGames : 1;
    ...
    if (this.profile.totalGamesPlayed >= 3) {
      ...
      const averageScore =
        this.profile.totalScore / this.profile.totalGamesPlayed;
   ...
   * 動的敵撃破時の処理
   */
  public handleDynamicEnemyDestroyed(
    isElite: boolean,
    threatLevel: number
  ): void {
    // 基本スコア
    ...
    if (isElite) {
      console.log(
        `Elite enemy defeated! Bonus: ${baseScore + threatBonus} points, 5 coins`
      );
    }
  }
