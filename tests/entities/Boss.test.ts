...
describe('Boss', () => {
  ...
  describe('初期化', () => {
    test('Bossが正しく初期化される', () => {
      ...
      expect(boss.getX()).toBe(
        testConfig.canvas.width / 2 - testConfig.boss.width / 2
      );
  ...
  describe('移動システム', () => {
    ...
    test('画面端で方向転換する - 右端', () => {
      ...
      expect(boss.getX()).toBeLessThanOrEqual(
        testConfig.canvas.width - boss.getWidth()
      );
    });
  ...
  describe('描画システム', () => {
    ...
    test('低体力時のフリッカーエフェクト', () => {
      // 体力を低く設定
      const lowHealthThreshold = Math.floor(
        testConfig.boss.initialHealth * 0.3
      );
      for (
        let i = 0;
        i < testConfig.boss.initialHealth - lowHealthThreshold;
        i++
      ) {
        boss.takeDamage();
      }
