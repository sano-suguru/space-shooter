...
export const UpgradeShop: React.FC<UpgradeShopProps> = ({
  ...
  const [currentCategory, setCurrentCategory] = useState<UpgradeCategory>('weapon');
  ...
  const filteredUpgrades = useMemo(() => {
    return availableUpgrades.filter(upgrade => upgrade.category === currentCategory);
  }, [availableUpgrades, currentCategory]);

  // カテゴリ変更ハンドラ
  const handleCategoryChange = useCallback((category: UpgradeCategory) => {
  ...
  return (
    ...
    >
      <Card 
        className="upgrade-shop-container"
        size="large"
      >
        {/* ショップヘッダー */}
