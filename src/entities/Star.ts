...
export class Star {
  ...
  constructor(randomProvider: IRandomProvider, config?: GameConfig) {
    ...
    if (this.starType === 'binary') {
      this.secondarySize =
        this.baseSize * (0.5 + this.randomProvider.random() * 0.5);
  ...
  private drawRegularStar(
    ...
    glowGradient.addColorStop(
      0.7,
      `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${alpha * 0.3})`
    );
  ...
  private drawVariableStar(
    ctx: CanvasRenderingContext2D,
    twinkleEffect: number
  ): void {
    const pulse = Math.sin(this.pulsePhase) * 0.5 + 0.5;
    const currentSize =
      this.baseSize * (0.5 + pulse * 0.8 + twinkleEffect * 0.3);
    const alpha =
