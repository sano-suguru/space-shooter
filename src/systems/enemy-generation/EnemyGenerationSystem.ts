import { EnemyGenerationRequest, DynamicEnemyConfig, DifficultyFactors } from "../types/EnemyGeneration";
...
export class EnemyGenerationSystem {
    ...
    constructor(eventEmitter: EventEmitter<EventMap>, randomProvider?: IRandomProvider) {
     ...
     * 複数の敵を一括生成
     */
    public generateEnemyBatch(requests: EnemyGenerationRequest[]): DynamicEnemyConfig[] {
        // 群れコンテキストを各リクエストに適用
        const updatedRequests = requests.map(request => this.updateFlockingContext(request));
     ...
     * 群れコンテキストを更新
     */
    private updateFlockingContext(request: EnemyGenerationRequest): EnemyGenerationRequest {
        const existingFlocks = Array.from(this.activeFlocks.keys());
        const preferredFlockSize = this.calculatePreferredFlockSize(request.baseType);
    ...
    private calculatePreferredFlockSize(enemyType: EnemyType): number {
        switch (enemyType) {
            case 'SMALL': return this.randomProvider.randomInt(3, 5);
            case 'MEDIUM': return this.randomProvider.randomInt(2, 4);
            case 'LARGE': return this.randomProvider.randomInt(1, 3);
            default: return 3;
        }
    }
    ...
    private selectEnemyTypeForEnvironment(environmentType: string): EnemyType {
        switch (environmentType) {
            case 'nebula':
                // 星雲では小型・中型敵が多い
                return this.randomProvider.randomChoice(['SMALL', 'MEDIUM'] as EnemyType[]);
            case 'planet':
                // 惑星近くでは大型敵が多い
                return this.randomProvider.randomChoice(['MEDIUM', 'LARGE'] as EnemyType[]);
            default:
                return this.randomProvider.randomChoice(['SMALL', 'MEDIUM', 'LARGE'] as EnemyType[]);
        }
    }
    ...
    public removeEnemyFromFlock(flockId: string, enemyId: string): void {
        ...
        if (flock) {
            const index = flock.findIndex(enemy => 
                `${enemy.position.x}_${enemy.position.y}` === enemyId
            );
            if (index !== -1) {
    ...
    public getGenerationStatistics(): {
        ...
        this.generationHistory.forEach(enemy => {
            ...
            if (enemy.difficultyFactors) {
                const avgMultiplier = (
                    (enemy.difficultyFactors.playerLevel * enemy.difficultyFactors.levelScaling) +
                    (enemy.difficultyFactors.currentWave * enemy.difficultyFactors.waveScaling)
                ) / 2;
        ...
        return {
            ...
            typeDistribution,
            eliteRatio: this.generationHistory.length > 0 ? eliteCount / this.generationHistory.length : 0,
            averageDifficulty: this.generationHistory.length > 0 ? totalDifficulty / this.generationHistory.length : 0,
    ...
    public getDebugInfo(): {
        ...
        recentEnemies: Array<ReturnType<EnemyGeneratorFactory['getDetailedEnemyInfo']>>;
        statistics: ReturnType<EnemyGenerationSystem['getGenerationStatistics']>;
    } {
        const flockInfo = Array.from(this.activeFlocks.entries()).map(([id, members]) => ({
            id,
