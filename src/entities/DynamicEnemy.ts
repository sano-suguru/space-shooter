...
export class DynamicEnemy extends Enemy {
  ...
  public update(deltaTime: number): void {
    this.dynamicAnimationPhase +=
      deltaTime * this.dynamicConfig.appearance.animationSpeed;
    ...
    if (
      playerPos &&
      this.behaviorComponent.shouldAttack(
        { x: this.x, y: this.y },
        this.dynamicConfig.behavior,
        playerPos,
        this.lastAttackTime,
  ...
  private drawDebugInfo(ctx: CanvasRenderingContext2D): void {
    ...
    if (this.dynamicConfig.flockId) {
      ...
      ctx.fillText(
        `Flock: ${this.dynamicConfig.flockId.slice(-8)}`,
        this.x,
   ...
   * 環境効果を適用
   */
  public applyEnvironmentalEffect(
