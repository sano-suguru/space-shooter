...
export class PlayerRenderer {
  ...
  private drawShip(
    ...
    ctx.fillStyle = invincible
      ? 'rgba(255, 0, 0, 0.5)'
      : this.config.player.colors.primary;
    ctx.beginPath();
  ...
  private drawEngineTrail(
    ...
    gradient.addColorStop(
      0,
      `rgba(100, 150, 255, ${0.8 + Math.sin(this.animationTime * 5) * 0.2})`
    );
    ...
    [-width / 3, width / 3].forEach(offset => {
      const sideGradient = ctx.createLinearGradient(
        ...
        engineY - 5,
        centerX + offset,
        engineY + sideTrailLength
      );
      sideGradient.addColorStop(
        0,
        `rgba(255, 100, 0, ${0.6 + Math.sin(this.animationTime * 4) * 0.2})`
      );
      ...
      ctx.lineTo(
        centerX + offset + sideTrailWidth / 4,
        engineY + sideTrailLength
      );
      ctx.lineTo(
        centerX + offset - sideTrailWidth / 4,
  ...
  private drawWeaponSystems(
    ...
    coreGlow.addColorStop(
      0,
      `rgba(255, 255, 100, ${0.8 + Math.sin(this.animationTime * 6) * 0.2})`
    );
