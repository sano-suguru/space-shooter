...
describe('AchievementManager', () => {
  ...
  let mockOnProfileUpdate: jest.MockedFunction<
    (profile: PlayerProfile) => void
  >;
  let mockOnAchievementUnlocked: jest.MockedFunction<
  ...
  describe('基本機能', () => {
    test('初期化時にプロファイルが正しく設定される', () => {
      ...
      expect(
        achievementManager.getIncompleteAchievements().length
      ).toBeGreaterThan(0);
    });
  ...
  describe('アチーブメント達成判定', () => {
    test('初回ゲーム完了でfirst_stepsアチーブメントが解除される', () => {
      ...
      expect(mockOnAchievementUnlocked).toHaveBeenCalledWith(
    ...
    test('スコア条件を満たすとscore_hunterアチーブメントが解除される', () => {
      ...
      const scoreHunter = results.find(
        r => r.achievement.id === 'score_hunter'
      );
    ...
    test('ゲームセッション統計を使用するアチーブメントが正しく判定される', () => {
      ...
      const perfectionist = results.find(
  ...
  describe('報酬システム', () => {
    ...
    test('アップグレード解除報酬が正しく適用される', () => {
      ...
      const results = achievementManager.checkAchievements();
      const enemySlayer = results.find(
        r => r.achievement.id === 'enemy_slayer'
      );
    ...
    test('レベルアップが経験値報酬により発生する', () => {
      ...
      const results = achievementManager.checkAchievements();
      const waveChampion = results.find(
        r => r.achievement.id === 'wave_champion'
      );
  ...
  describe('統計機能', () => {
    ...
    test('進捗追跡が正しく動作する', () => {
      ...
      const progress =
        achievementManager.getAchievementProgress('score_hunter');
  ...
  describe('フィルタリング機能', () => {
    test('表示用アチーブメント取得（隠しアチーブメント除外）', () => {
      const visibleAchievements =
        achievementManager.getDisplayAchievements(false);

      expect(visibleAchievements.every(a => !a.hidden)).toBe(true);
    });

    test('カテゴリ別フィルタリングが動作する', () => {
      const combatAchievements = achievementManager.getDisplayAchievements(
        false,
        'combat'
      );
    ...
    test('難易度別フィルタリングが動作する', () => {
      const goldAchievements = achievementManager.getDisplayAchievements(
        false,
        undefined,
        'gold'
      );
    ...
    test('達成可能なアチーブメント取得が動作する', () => {
      ...
      const achievableAchievements =
        achievementManager.getAchievableAchievements();
      ...
      achievableAchievements.forEach(achievement => {
        if (achievement.progressTracker) {
          ...
          expect(progress.current / progress.required).toBeGreaterThanOrEqual(
            0.5
          );
        }
      });
  ...
  describe('強制解除機能（デバッグ用）', () => {
    ...
    test('存在しないアチーブメントIDでnullが返される', () => {
      const result = achievementManager.forceUnlockAchievement(
  ...
  describe('エラーハンドリング', () => {
    ...
    test('プログレストラッカーのないアチーブメントで進捗取得を試みるとnullが返される', () => {
      ...
      const progress =
        achievementManager.getAchievementProgress('perfectionist');
