...
export class GameObjectFactory {
  ...
  constructor(
    ...
    // 動的敵生成システムの初期化（オプション）
    if (eventEmitter) {
      this.enemyGenerationSystem = new EnemyGenerationSystem(
        eventEmitter,
        randomProvider
      );
    ...
    if (!this.isDynamicEnemyEnabled()) {
      enemyTypes.forEach(({ type, count, positions }) => {
        for (let i = 0; i < count; i++) {
          ...
          if (position) {
            enemies.push(
              this.createEnemyAtPosition(type, position.x, position.y, game)
            );
          } else {
  ...
  createEnvironmentalEnemies(
    ...
    if (!this.isDynamicEnemyEnabled()) {
      for (let i = 0; i < count; i++) {
        const randomType = this.randomProvider.randomChoice([
          'SMALL',
          'MEDIUM',
          'LARGE',
        ] as EnemyType[]);
    ...
    const dynamicConfigs =
      this.enemyGenerationSystem!.generateEnvironmentalEnemies(
        environmentType,
        count,
        difficultyFactors
      );
  ...
  createPowerUp(): PowerUp {
    const x = randomRange(
      0,
      this.config.canvas.width - this.config.powerup.width
    );
  ...
  private createFallbackEnemy(
    ...
    console.warn(
      'Dynamic enemy generation failed, falling back to traditional enemy creation'
    );
    return this.createEnemyAtPosition(type, position.x, position.y, game);
  }
