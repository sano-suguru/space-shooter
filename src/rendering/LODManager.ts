...
export class LODManager {
    ...
    constructor(performanceMonitor: PerformanceMonitor, customThresholds?: Partial<LODThresholds>) {
     ...
     * LOD設定をカスタマイズ
     */
    public customizeLODSettings(level: LODLevel, settings: Partial<LODSettings>): void {
        this.lodConfiguration[level] = { ...this.lodConfiguration[level], ...settings };
    }
    ...
    public logLODInfo(): void {
        ...
        console.log(`🎨 Particle Multiplier: ${(stats.settings.particleMultiplier * 100).toFixed(0)}%`);
        console.log(`🔍 Detail Level: ${(stats.settings.detailLevel * 100).toFixed(0)}%`);
        console.log(`🔄 Update Frequency: ${(stats.settings.updateFrequency * 100).toFixed(0)}%`);
        console.log(`✨ Effect Intensity: ${(stats.settings.effectIntensity * 100).toFixed(0)}%`);
        console.log(`🎬 Animation Quality: ${(stats.settings.animationQuality * 100).toFixed(0)}%`);
        console.groupEnd();
    ...
    private calculateRecommendedLOD(metrics: PerformanceMetrics): LODLevel {
        ...
        const renderTimeScore = this.calculateRenderTimeScore(metrics.averageRenderTime);
        
        // 重み付き平均（FPSを最重要視）
        const totalScore = (fpsScore * 0.5) + (memoryScore * 0.3) + (renderTimeScore * 0.2);
    ...
    private changeLODLevel(newLevel: LODLevel): void {
        ...
        console.log(`🎯 LOD Level changed: ${this.previousLevel} → ${this.currentLevel}`);
