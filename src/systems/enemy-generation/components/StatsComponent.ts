import { EnemyStats, DifficultyFactors, DifficultyModifiers } from "../../types/EnemyGeneration";
import { ENEMY_BASE_TEMPLATES, VARIATION_RANGES, ELITE_MODIFIERS } from "../../../data/EnemyTemplates";
...
export class StatsComponent {
    ...
    public generateBaseStats(enemyType: EnemyType): EnemyStats {
        ...
        return {
            health: this.applyVariation(baseStats.health, VARIATION_RANGES.stats.health),
            speed: this.applyVariation(baseStats.speed, VARIATION_RANGES.stats.speed),
            attackPower: this.applyVariation(baseStats.attackPower, VARIATION_RANGES.stats.attackPower),
            defense: this.applyVariation(baseStats.defense, VARIATION_RANGES.stats.defense),
            fireRate: this.applyVariation(baseStats.fireRate, VARIATION_RANGES.stats.fireRate),
            accuracy: this.applyVariation(baseStats.accuracy, VARIATION_RANGES.stats.accuracy),
            experienceReward: this.applyVariation(baseStats.experienceReward, VARIATION_RANGES.stats.experienceReward),
            scoreValue: this.applyVariation(baseStats.scoreValue, VARIATION_RANGES.stats.scoreValue)
        };
    }
    ...
    public generateAdjustedStats(
        ...
        return {
            ...
            attackPower: Math.round(baseStats.attackPower * modifiers.attackMultiplier),
            ...
            accuracy: Math.min(0.95, baseStats.accuracy * (1 + (modifiers.attackMultiplier - 1) * 0.3)), // 精度も少し向上
            experienceReward: Math.round(baseStats.experienceReward * modifiers.healthMultiplier),
    ...
    public generateEliteStats(baseStats: EnemyStats): EnemyStats {
        ...
        return {
            ...
            experienceReward: Math.round(baseStats.experienceReward * modifiers.experienceMultiplier),
    ...
    public applyEnvironmentalEffects(
        ...
        switch (effectType) {
            case 'speed_boost':
                modifiedStats.speed = Math.round(baseStats.speed * (1 + intensity * 0.2));
                break;
            case 'damage_boost':
                modifiedStats.attackPower = Math.round(baseStats.attackPower * (1 + intensity * 0.15));
                break;
            case 'shield_regen':
                modifiedStats.defense = Math.round(baseStats.defense * (1 + intensity * 0.1));
                modifiedStats.health = Math.round(baseStats.health * (1 + intensity * 0.05));
                break;
            case 'stealth':
                // ステルス効果は視覚的なもので、能力値には直接影響しない
                // ただし、少し速度を上げて回避能力を向上
                modifiedStats.speed = Math.round(baseStats.speed * (1 + intensity * 0.1));
                break;
        }
     ...
     * 難易度修正値を計算
     */
    private calculateDifficultyModifiers(factors: DifficultyFactors): DifficultyModifiers {
        // 基本倍率の計算
        const baseMultiplier = factors.baseMultiplier + 
        ...
        return {
            healthMultiplier: Math.max(1.0, adjustedMultiplier * this.randomProvider.randomRange(0.9, 1.3)),
            speedMultiplier: Math.max(0.5, adjustedMultiplier * this.randomProvider.randomRange(0.8, 1.4)),
            attackMultiplier: Math.max(1.0, adjustedMultiplier * this.randomProvider.randomRange(0.9, 1.2)),
     ...
     * 能力値に変動を適用
     */
    private applyVariation(baseValue: number, range: { min: number; max: number }): number {
     ...
     * 能力値の比較（デバッグ用）
     */
    public compareStats(stats1: EnemyStats, stats2: EnemyStats): {
        healthRatio: number;
        speedRatio: number;
        attackRatio: number;
        defenseRatio: number;
        ...
        return {
            ...
            overallPowerRatio: this.calculateOverallPower(stats1) / this.calculateOverallPower(stats2)
        };
    }
    ...
    public getStatsInfo(stats: EnemyStats): {
        ...
        const survivalTime = stats.health / Math.max(1, playerBaseDamage - stats.defense);
