...
export class PerformanceMonitor {
  ...
  public getDetailedStats(): {
    ...
    return {
      ...
      renderTime: {
        ...
        min:
          this.renderTimeHistory.length > 0
            ? Math.min(...this.renderTimeHistory)
            : 0,
        max:
          this.renderTimeHistory.length > 0
            ? Math.max(...this.renderTimeHistory)
            : 0,
      },
      memory: {
        current:
        ...
        min:
          this.memoryHistory.length > 0 ? Math.min(...this.memoryHistory) : 0,
        max:
          this.memoryHistory.length > 0 ? Math.max(...this.memoryHistory) : 0,
  ...
  public logPerformanceInfo(): void {
    ...
    console.log(
      `📊 FPS: ${stats.fps.current.toFixed(1)} (avg: ${stats.fps.average.toFixed(1)}, stability: ${(stats.fps.stability * 100).toFixed(1)}%)`
    );
    console.log(
      `⏱️ Frame Time: ${stats.frameTime.current.toFixed(2)}ms (avg: ${stats.frameTime.average.toFixed(2)}ms)`
    );
    console.log(
      `🎨 Render Time: ${stats.renderTime.current.toFixed(2)}ms (avg: ${stats.renderTime.average.toFixed(2)}ms)`
    );
    console.log(
      `💾 Memory: ${stats.memory.current.toFixed(1)}MB (trend: ${stats.memory.trend})`
    );
  ...
  private checkFPSWarnings(fps: number): void {
    if (fps < this.thresholds.fps.critical) {
      this.addWarning(
        'fps',
        'high',
        `Critical FPS drop: ${fps.toFixed(1)} FPS`,
        fps,
  ...
  private checkRenderTimeWarnings(renderTime: number): void {
    if (renderTime > this.thresholds.renderTime.critical) {
      this.addWarning(
        'render-time',
        'high',
        `Critical render time: ${renderTime.toFixed(2)}ms`,
        renderTime,
        this.thresholds.renderTime.critical
      );
    } else if (renderTime > this.thresholds.renderTime.warning) {
      this.addWarning(
        'render-time',
        'medium',
        `High render time: ${renderTime.toFixed(2)}ms`,
        renderTime,
