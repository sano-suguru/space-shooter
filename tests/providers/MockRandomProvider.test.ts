...
describe('MockRandomProvider', () => {
  ...
  describe('setSeed()', () => {
    it('同じシード値では同じ値が生成される', () => {
      ...
      const values1 = Array(5)
        .fill(0)
        .map(() => provider1.random());
      const values2 = Array(5)
        .fill(0)
        .map(() => provider2.random());
  ...
  describe('randomChoice()', () => {
    ...
    it('空配列の場合はエラーを投げる', () => {
      expect(() => randomProvider.randomChoice([])).toThrow(
  ...
  describe('randomChance()', () => {
    ...
    it('確率が範囲外の場合はエラーを投げる', () => {
      expect(() => randomProvider.randomChance(-0.1)).toThrow(
        'Probability must be between 0 and 1'
      );
      expect(() => randomProvider.randomChance(1.1)).toThrow(
        'Probability must be between 0 and 1'
      );
    });
