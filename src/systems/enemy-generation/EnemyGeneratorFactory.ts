import { DynamicEnemyConfig, EnemyGenerationRequest } from "../types/EnemyGeneration";
...
export class EnemyGeneratorFactory {
     ...
     * 動的敵設定を生成
     */
    public generateDynamicEnemy(request: EnemyGenerationRequest): DynamicEnemyConfig {
        // 難易度修正値を計算
        const difficultyModifiers = this.difficultySystem.calculateSmoothDifficultyModifiers(request.difficultyFactors);

        // エリート敵かどうかを判定
        const isElite = this.randomProvider.randomChance(difficultyModifiers.eliteEnemyChance);

        // 基本設定を取得
        const baseTemplate = ENEMY_BASE_TEMPLATES[request.baseType];

        // 各コンポーネントを生成
        let appearance = this.appearanceComponent.generateRandomAppearance(baseTemplate.appearance);
        let stats = this.statsComponent.generateBaseStats(request.baseType);
        let behavior = this.behaviorComponent.generateRandomBehavior(request.baseType);
        ...
        // 難易度調整を適用
        stats = this.statsComponent.generateAdjustedStats(stats, request.difficultyFactors);
        ...
        if (environmentalEffects.length > 0) {
            environmentalEffects.forEach(effect => {
                stats = this.statsComponent.applyEnvironmentalEffects(stats, effect.effectType, effect.intensity);
                behavior = this.behaviorComponent.adjustForEnvironment(behavior, effect.triggerZone);
            });
        }
    ...
    private generateFlockInfo(
        ...
        // 既存の群れに参加するかチェック
        if (request.flockingContext?.existingFlocks && request.flockingContext.existingFlocks.length > 0) {
            const joinChance = behavior.flockingTendency * 0.8;
            if (this.randomProvider.randomChance(joinChance)) {
                const flockId = this.randomProvider.randomChoice(request.flockingContext.existingFlocks);
                return { flockId };
            }
        }
    ...
    private assignFlockLeader(flockMembers: DynamicEnemyConfig[]): void {
        ...
        if (!leader) {
            leader = flockMembers.reduce((strongest, current) => {
                const strongestPower = this.statsComponent.calculateOverallPower(strongest.stats);
                const currentPower = this.statsComponent.calculateOverallPower(current.stats);
                return currentPower > strongestPower ? current : strongest;
            });
        }

        // リーダーの行動を調整
        leader.behavior = this.behaviorComponent.generateLeaderBehavior(leader.behavior);

        // 他のメンバーをフォロワーに設定
        flockMembers.forEach(member => {
            if (member !== leader) {
                member.leaderId = `${leader.position.x}_${leader.position.y}_${Date.now()}`;
                member.behavior = this.behaviorComponent.generateFollowerBehavior(member.behavior);
            }
        });
    }
    ...
    public getGenerationStats(enemies: DynamicEnemyConfig[]): {
        ...
        // 平均的な難易度修正値を計算
        const sampleDifficulty = enemies.length > 0 ? 
            this.difficultySystem.calculateSmoothDifficultyModifiers(enemies[0].difficultyFactors!) :
            { healthMultiplier: 1, speedMultiplier: 1, attackMultiplier: 1, specialAbilityChance: 0, eliteEnemyChance: 0 };
        
        const difficultyLevel = this.difficultySystem.getDifficultyLevel(sampleDifficulty);
    ...
    public getDetailedEnemyInfo(enemy: DynamicEnemyConfig): {
        ...
        return {
            ...
            attack: this.attackComponent.getAttackInfo(enemy.attack, enemy.stats.attackPower),
            flockInfo: {
