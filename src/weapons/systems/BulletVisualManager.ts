...
export class BulletVisualManager {
  ...
  /**
   * 追尾状態の特別なエフェクトを描画（軽量版）
   */
  private renderTrackingEffects(
    bullet: Bullet,
    context: BulletRenderContext
  ): void {
    // パフォーマンスモードでは追尾エフェクトをスキップ
    if (this.performanceMode) {
      return;
    }
    ...
    // 精密射撃弾道の場合のみ追尾エフェクトを描画
    if (!trajectory || trajectory.getType().toString() !== 'precision') {
      return;
    }

    // 追尾状態をチェック（型安全性のためのチェック）
    const precisionTrajectory = trajectory as {
      isTracking?: () => boolean;
      getTargetPosition?: () => { x: number; y: number } | undefined;
    };

    const isTracking = precisionTrajectory.isTracking?.() ?? false;
    const targetPosition = precisionTrajectory.getTargetPosition?.();
    ...
    // 軽量な追尾線のみ描画
    this.renderLightweightTrackingLine(context, targetPosition);
  ...
  private renderTrackingLine(
    ...
    gradient.addColorStop(0, 'rgba(255, 255, 0, 0.8)'); // 黄色
   ...
   * 追尾オーラを描画
   */
  private renderTrackingAura(context: BulletRenderContext): void {
    const { ctx } = context;
    const time = Date.now() * 0.005;
    const pulseSize = 1 + Math.sin(time) * 0.3;
    const radius = Math.max(context.width, context.height) * pulseSize;
    ...
    gradient.addColorStop(0, 'rgba(255, 255, 0, 0.6)'); // 黄色のオーラ
    ...
    for (let i = 0; i < sparkCount; i++) {
      const angle = (i / sparkCount) * Math.PI * 2 + time;
      const sparkDistance = radius * 0.7;
      const sparkX = context.x + Math.cos(angle) * sparkDistance;
      const sparkY = context.y + Math.sin(angle) * sparkDistance;
   ...
   * 軽量な追尾線を描画
   */
  private renderLightweightTrackingLine(
    context: BulletRenderContext,
    targetPosition: { x: number; y: number }
  ): void {
    const { ctx } = context;
    ...
    // シンプルな追尾線（重い処理を避ける）
    ctx.strokeStyle = 'rgba(255, 255, 0, 0.6)'; // 黄色
    ctx.lineWidth = 1;
    ctx.setLineDash([3, 3]); // 短い点線
