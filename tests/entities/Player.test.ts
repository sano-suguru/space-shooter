...
describe('Player', () => {
  ...
  describe('初期化', () => {
    test('プレイヤーが正しい初期位置に配置される', () => {
      ...
      const expectedX =
        testConfig.canvas.width / 2 - testConfig.player.width / 2;
      const expectedY =
        testConfig.canvas.height - testConfig.player.height - 10;
  ...
  describe('移動処理', () => {
    ...
    test('画面境界で停止する - 右端', () => {
      ...
      const testConfig = createTestConfig();
      expect(player.getPosition().x).toBe(
        testConfig.canvas.width - testConfig.player.width
      );
    });
    ...
    test('画面境界で停止する - 下端', () => {
      ...
      const testConfig = createTestConfig();
      expect(player.getPosition().y).toBe(
        testConfig.canvas.height - testConfig.player.height
      );
    });
  });
  ...
  describe('アニメーション・パーティクル', () => {
    ...
    test('スラスターパーティクルが生成される', () => {
      ...
      mockRandomProvider.random = jest
        .fn()
  ...
  describe('設定注入機能', () => {
    ...
    test('設定に基づく境界制限', () => {
      ...
      expect(configPlayer.getPosition().x).toBe(
        testConfig.canvas.width - testConfig.player.width
      );
    });
    ...
    test('設定に基づく無敵時間', () => {
      ...
      jest
        .spyOn(Date, 'now')
        .mockReturnValue(1000 + testConfig.player.invincibilityTime + 10);
      configPlayer.update(0.016);
    ...
    test('設定の動的変更', () => {
      const newService = new PowerUpEffectService(
        createTestConfig({
          player: { fireRate: 300 },
        })
      );
