...
export class AchievementManager {
     ...
     */
    forceUnlockAchievement(achievementId: string): AchievementUnlockResult | null {
    ...
    private unlockAchievement(achievement: Achievement): AchievementUnlockResult {
        ...
        if (achievement.reward.unlockUpgrade) {
            if (!this.playerProfile.unlockedUpgrades.includes(achievement.reward.unlockUpgrade)) {
                this.playerProfile.unlockedUpgrades.push(achievement.reward.unlockUpgrade);
    ...
    getAchievementStats(): AchievementStats {
        ...
        const completedAchievements = this.playerProfile.completedAchievements.length;
        ...
        const bronzeCount = getAchievementsByDifficulty('bronze').filter(a => completedIds.has(a.id)).length;
        const silverCount = getAchievementsByDifficulty('silver').filter(a => completedIds.has(a.id)).length;
        const goldCount = getAchievementsByDifficulty('gold').filter(a => completedIds.has(a.id)).length;
        const platinumCount = getAchievementsByDifficulty('platinum').filter(a => completedIds.has(a.id)).length;

        // 隠しアチーブメント発見数
        const hiddenFound = getHiddenAchievements().filter(a => completedIds.has(a.id)).length;
    ...
    getAchievableAchievements(): Achievement[] {
        return this.getIncompleteAchievements()
            .filter(achievement => {
    ...
    getNextTargetAchievement(): Achievement | null {
        const incomplete = this.getIncompleteAchievements()
            .filter(achievement => achievement.progressTracker && !achievement.hidden);
