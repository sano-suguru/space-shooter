...
export class UpgradeManager {
  ...
  private recalculateEquippedUpgrades(): void {
    ...
    // 装備中の各アップグレードを処理
    for (const [upgradeId, level] of Object.entries(
      this.currentProfile.equippedUpgrades
    )) {
      if (level > 0) {
        const config = getUpgradeConfig(upgradeId);
        if (config) {
   ...
   */
  getAvailableUpgradesByCategory(
  ...
  canPurchaseUpgrade(upgradeId: string): boolean {
    ...
    return (
      isUpgradeUnlocked(upgradeId, this.currentProfile) &&
  ...
  getUpgradeStats(): {
    ...
    const averageUpgradeLevel =
      totalUpgradesOwned > 0 ? totalLevels / totalUpgradesOwned : 0;
   ...
   */
  getEffectValue<K extends keyof UpgradeEffect>(
    effectType: K
  ): UpgradeEffect[K] | undefined {
