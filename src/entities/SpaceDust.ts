...
export class SpaceDust {
    ...
    private createDustParticles(): void {
        ...
        for (let i = 0; i < this.particleCount; i++) {
            const particle = globalParticlePoolManager.getParticle<DustParticle>(this.poolName);
            if (particle) {
    ...
    private initializeDustParticle(particle: DustParticle): void {
        ...
        particle.color = colors[Math.floor(this.randomProvider.random() * colors.length)];
