...
export class MeteorShower {
  ...
  private createMeteor(): MeteorParticle {
    ...
    const startX =
      centerX - this.direction.x * spawnDistance + perpendicular.x * spread;
    const startY =
      centerY - this.direction.y * spawnDistance + perpendicular.y * spread;
  ...
  public update(deltaTime: number): void {
    ...
    this.meteors.forEach(meteor => {
      ...
      meteor.trail.forEach((point, index) => {
        point.alpha =
          (meteor.alpha * (meteor.trailLength - index)) / meteor.trailLength;
      });
    ...
    this.meteors = this.meteors.filter(meteor => {
      return (
        meteor.alpha > 0 &&
        meteor.x >= -50 &&
        meteor.x <= this.config.canvas.width + 50 &&
        meteor.y >= -50 &&
        meteor.y <= this.config.canvas.height + 50
      );
    });
    ...
    this.meteors.forEach(meteor => {
      ...
      if (meteor.trail.length > 1) {
        for (let i = 0; i < meteor.trail.length - 1; i++) {
          ...
          gradient.addColorStop(
            1,
            `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${point.alpha * 0.3})`
          );
      ...
      glowGradient.addColorStop(
        0,
        `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${meteor.alpha})`
      );
      glowGradient.addColorStop(
        0.7,
        `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${meteor.alpha * 0.5})`
      );
