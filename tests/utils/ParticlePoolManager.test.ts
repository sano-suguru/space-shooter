import {
  ParticlePoolManager,
  PooledParticle,
} from '../../src/utils/ParticlePoolManager';
...
describe('ParticlePoolManager', () => {
  ...
  describe('プール登録と基本操作', () => {
    test('パーティクルプールを正常に登録できる', () => {
      poolManager.registerPool('test-pool', () => new TestParticle(), {
        initialSize: 10,
        maxSize: 20,
        particleType: 'test',
      });
  ...
  describe('エラーハンドリング', () => {
    test('存在しないプールからのパーティクル取得はnullを返す', () => {
      const particle =
        poolManager.getParticle<TestParticle>('non-existent-pool');
  ...
  describe('リソースクリーンアップ', () => {
    ...
    test('全プールクリアが正常に動作する', () => {
      poolManager.registerPool('test-pool-1', () => new TestParticle(), {
        initialSize: 5,
        maxSize: 10,
        particleType: 'test',
      });

      poolManager.registerPool('test-pool-2', () => new TestParticle(), {
        initialSize: 5,
        maxSize: 10,
        particleType: 'test',
      });
