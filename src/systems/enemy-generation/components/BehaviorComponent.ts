...
import { ENEMY_BASE_TEMPLATES, VARIATION_RANGES, ADVANCED_BEHAVIOR_PATTERNS } from "../../../data/EnemyTemplates";
...
export class BehaviorComponent {
    ...
    public generateRandomBehavior(enemyType: EnemyType): BehaviorConfig {
        ...
        const availablePatterns: BehaviorPattern[] = ['straight', 'zigzag', 'sine', 'spiral', 'aggressive_chase'];
        const selectedPattern = this.randomProvider.randomChoice(availablePatterns);

        return {
            pattern: selectedPattern,
            aggressiveness: this.applyVariation(baseBehavior.aggressiveness, VARIATION_RANGES.behavior.aggressiveness),
            flockingTendency: this.applyVariation(baseBehavior.flockingTendency, VARIATION_RANGES.behavior.flockingTendency),
            environmentalAwareness: this.applyVariation(baseBehavior.environmentalAwareness, VARIATION_RANGES.behavior.environmentalAwareness)
        };
    }
    ...
    public generateEliteBehavior(baseBehavior: BehaviorConfig): BehaviorConfig {
        return {
            ...
            environmentalAwareness: Math.min(1.0, baseBehavior.environmentalAwareness + 0.2)
        };
    }
    ...
    public updatePosition(
        ...
        // 基本移動パターンの適用
        newPos = this.applyMovementPattern(newPos, behavior.pattern, deltaTime, speed, animationPhase);

        // 群れ行動の適用
        if (behavior.flockingTendency > 0 && (flockCenter || nearbyEnemies)) {
            newPos = this.applyFlockingBehavior(newPos, behavior, flockCenter, nearbyEnemies);
        }

        // 積極性に基づくプレイヤー追跡
        if (behavior.aggressiveness > 0.5 && playerPos) {
            newPos = this.applyAggressiveBehavior(newPos, behavior, playerPos, deltaTime, speed);
        }
    ...
    private applyMovementPattern(
        ...
        switch (pattern) {
            ...
            case 'spiral':
                ...
                const spiralRadius = spiralConfig.spiralRadius * Math.sin(animationPhase * 0.5);
    ...
    private applyFlockingBehavior(
        ...
        if (nearbyEnemies && nearbyEnemies.length > 0) {
            ...
            nearbyEnemies.forEach(enemyPos => {
                ...
                if (distance > 0 && distance < 50) { // 50ピクセル以内で分離
    ...
    private applyAggressiveBehavior(
        ...
        if (distance > 0 && distance < aggressiveConfig.attackRange) {
            // 攻撃範囲内でプレイヤーに向かう
            const chaseStrength = aggressiveness * aggressiveConfig.chaseSpeed * deltaTime;
    ...
    public generateLeaderBehavior(baseBehavior: BehaviorConfig): BehaviorConfig {
        return {
            ...
            environmentalAwareness: Math.min(1.0, baseBehavior.environmentalAwareness + 0.3)
        };
    }
     ...
     * フォロワー行動の生成
     */
    public generateFollowerBehavior(baseBehavior: BehaviorConfig): BehaviorConfig {
        return {
            ...baseBehavior,
    ...
    public validateBehavior(behavior: BehaviorConfig): BehaviorConfig {
        return {
            ...
            environmentalAwareness: Math.max(0, Math.min(1, behavior.environmentalAwareness))
        };
    }
    ...
    public getBehaviorDescription(behavior: BehaviorConfig): string {
        ...
        const aggressivenessLevel = behavior.aggressiveness > 0.7 ? '高' : 
                                  behavior.aggressiveness > 0.4 ? '中' : '低';
        const flockingLevel = behavior.flockingTendency > 0.7 ? '高' : 
                             behavior.flockingTendency > 0.4 ? '中' : '低';
