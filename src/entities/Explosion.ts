...
export class Explosion {
  ...
  private getExplosionColor(
    type: 'core' | 'spark' | 'smoke' | 'ember' = 'core'
  ): string {
    switch (type) {
      case 'core':
  ...
  public update(deltaTime: number): void {
    ...
    this.shockWaves.forEach(shockWave => {
      shockWave.radius = Math.max(
        0,
        shockWave.radius + (shockWave.maxRadius / this.duration) * 2
      );
      shockWave.alpha = Math.max(
        0,
        0.8 - Math.max(0, shockWave.radius) / Math.max(1, shockWave.maxRadius)
      );
    });
      ...
      switch (particle.type) {
        case 'core':
          particle.radius = Math.max(
            0,
            particle.initialRadius * Math.max(0, lifeRatio)
          );
          break;
        case 'spark':
          particle.radius = Math.max(
            0,
            particle.initialRadius * Math.max(0, lifeRatio * 0.5 + 0.5)
          );
          break;
        case 'smoke':
          particle.radius = Math.max(
            0,
            particle.initialRadius * Math.max(0, 2 - lifeRatio)
          );
          break;
        case 'ember':
          particle.radius = Math.max(
