...
describe('PowerUp', () => {
  ...
  describe('画面内判定', () => {
    ...
    test('画面下端ギリギリのPowerUp', () => {
      const edgePowerUp = new PowerUp(
        100,
        testConfig.canvas.height - 1,
        testConfig
      );
    ...
    test('画面を完全に出たPowerUp', () => {
      const offScreenPowerUp = new PowerUp(
        100,
    ...
    test('画面下端ちょうどのPowerUp', () => {
      const bottomPowerUp = new PowerUp(
        100,
        testConfig.canvas.height,
        testConfig
      );
  ...
  describe('統合テスト', () => {
    ...
    test('複数PowerUpの独立動作', () => {
      ...
      expect(
        types.every(type =>
          ['RAPID_FIRE', 'TRIPLE_SHOT', 'SHIELD'].includes(type)
        )
      ).toBe(true);
    });
  });
